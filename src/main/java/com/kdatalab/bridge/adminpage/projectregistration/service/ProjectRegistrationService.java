...
public class ProjectRegistrationService {
    ...
    public List<TaskDto> getProjectWithTasks(Long projectId) {
        return projectRegistrationRepository.selectTasksByProjectId(projectId);
    }

    public boolean existProjectName(Integer projectId, String projectName) {
        int cnt = projectRegistrationRepository.existProjectName(projectId, projectName.toLowerCase());
        return cnt > 0;
    }
    ...
    public ProjectRegistrationDto getProjectDetailsById(Integer projectId) {
        ...
        dto.setFileCount(projectDetails.get("FILE_COUNT") == null ? 0 : Integer.parseInt(projectDetails.get("FILE_COUNT").toString()));
        return dto;
    }
    ...
    public BiFunction<List<TaskDto>, Integer, String> tasksToJson = (tasks, projectId) -> {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            ...
            if (t.getDtlSeq() != null) {
                ...
                if (t.getLoginId() == null || taskDto.getTaskDtlProg()>0) loginId = String.format("\"%s\"",taskDto.getLoginId()); // progress > 0 dan bo'lganda uni bajaruvchisini o'zgartirib bo'lmaydi (ayyor dasturchilar uchun)
                ...
                else loginId = String.format("\"%s\"",taskDto.getLoginId()); // agar yuqoridagi shartlardan o'tmasa demak o'zgartirilmaydi

                if (t.getQcId() == null) qcId = String.format("\"%s\"",taskDto.getQcId());   // yani select option da disabled bo'lsa null keladi
                ...
                else if(taskDto.getTaskDtlProg()>0 && taskDto.getQcId() != null) qcId = String.format("\"%s\"",taskDto.getQcId());  // yani progress 0 dan katta va oldin bu taskni tekshiruvchisi tanlagan bo'lgan holatda o'zgartirmaydi(ayyor dasturchilar uchun check)
    ...
    public void saveAssignedUsers(List<TaskDto> dto, String name, int projectId) {
        String tasks = tasksToJson.apply(dto, projectId);
    ...
    public Integer getWorkingTaskCount(Integer projectId) {
        return projectRegistrationRepository.getWorkingTaskCount(projectId);
    }
}
