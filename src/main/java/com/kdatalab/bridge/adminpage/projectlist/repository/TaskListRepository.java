...
public interface TaskListRepository extends JpaRepository<RootEntity,Integer> {
    ...
    @Query(value = "select ifnull(AVG(TASK_DTL_PROG),0) from TB_TASK_DTL where EDU_SEQ = ?1", nativeQuery = true)
    Integer getOverallProgressRateByProject(Integer projectId);

    @Query(value = "select ifnull(sum(COMPTPOINT),0) from TB_EDU_RESULT where EDU_SEQ = ?1", nativeQuery = true)
